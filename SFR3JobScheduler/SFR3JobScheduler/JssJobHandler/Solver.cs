...
namespace SFR3JobScheduler.JssJobHandler
{
	public class Solver
	{
		...
		public Solution solve()
		{
            ...
            //                while (true)
            //                {
            //                    var possibleJobs = solution.UnassignedJobs.Values
            //                        .Where(job =>
            //                            job.RequiredSkill == tech.Skill &&
            ...
            //                        if (finishTime > job.SLA) continue;

            //                        var slack = (job.SLA - finishTime).TotalMinutes;
            //                        var cost = distance + slack * 0.1;
            ...
            foreach (var tech in solution.technicians)
            {
                ...
                while (true)
                {
                    ...
                    // Optional: Check for overlap safety (paranoia mode)
                    //bool overlaps = assignedJobs.Values.Any(j =>
                    //{
                    //    var start = j.StartTime;
                    //    var end = j.StartTime + j.DurationEstimate;
                    //    var newStart = bestJob.StartTime;
                    //    var newEnd = bestJob.StartTime + bestJob.DurationEstimate;
                    //    return !(newEnd <= start || newStart >= end);
                    //});

                    //if (overlaps)
                    //{
                    //    // skip if somehow it overlaps (paranoia mode)
                    //    break;
                    //}
        ...
        {
            //var last_job_tmp = solution.Assignments[technician.Id].LastOrDefault();
            
            //var last_job = last_job_tmp.Value;
            //var lastLocation = last_job == null ? technician.Location : last_job.Location;
